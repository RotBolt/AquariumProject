package practiceTime.lessonFunctionalManipulation
...
class Location(val width: Int = 4, val height: Int = 4) {
    ...
    fun updateLocation(direction: Direction) {
        ...
        when(direction) {
            Direction.NORTH -> currentLocation = Pair(currentLocation.first, (currentLocation.second + 1).rem(height))
            Direction.SOUTH -> currentLocation = Pair(currentLocation.first, (currentLocation.second - 1).absoluteValue.rem(height))
            Direction.EAST -> currentLocation = Pair((currentLocation.first + 1).rem(width), currentLocation.second)
            Direction.WEST -> currentLocation = Pair((currentLocation.first - 1).absoluteValue.rem(width), currentLocation.second)
            else -> { }
        }
    }
...
class Game {
    ...
    private val end = { path.add(Direction.END); println("Game Over: $path"); path.clear(); false }

    fun makeMove(command: String?) {

        when (command) {
            "n" -> {
                move(north); map.updateLocation(Direction.NORTH)
            }
            "s" -> {
                move(south); map.updateLocation(Direction.SOUTH)
            }
            "e" -> {
                move(east); map.updateLocation(Direction.EAST)
            }
            "w" -> {
